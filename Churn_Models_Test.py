...
class test_CC_Offer_Terms(unittest.TestCase):
    def test_json_reading(self):
        offername = 'Chase Freedom'
        folder = 'JSON/'
        jsonwrite = Offer.write_to_json_file((CC_Offer_Terms(offername, 150, 1000, 4, datetime.datetime.strptime('04202017', '%m%d%Y'), datetime.timedelta(days = float(60)), datetime.timedelta(days = float(30)), 12, 6)), f'{folder}{offername}')
        jsonread = Offer.read_from_json_file(f'{folder}{offername}.json')
        ...
        os.remove(f'{folder}{offername}.json')
...
class test_SA_Offer_Terms(unittest.TestCase):
    def test_json_readwrite(self):
        ...
        folder = 'JSON/'
        jsonwrite = Offer.write_to_json_file(SA_Offer_Terms(offername, 200, 10000, 5, 3, 10000, datetime.datetime.strptime('07172017', '%m%d%Y'), datetime.timedelta(days = float(60)), datetime.timedelta(days= float(30)), 12, 6), f'{folder}{offername}')
        jsonread = Offer.read_from_json_file(f'{folder}{offername}.json')
        self.assertEqual(offername, jsonread.offer_name)
...
class test_CA_Offer_Terms(unittest.TestCase):
    def test_json_readwrite(self):
        ...
        folder = 'JSON/'
        jsonwrite = Offer.write_to_json_file(CA_Offer_Terms(offername, 200, 100000, 50000, 5, 10, 2500, 1250, False, datetime.datetime.strptime('04202017', '%m%d%Y'), datetime.timedelta(days = float(60)), datetime.timedelta(days = float(30)), 12, 6), f'{folder}{offername}')
        jsonread = Offer.read_from_json_file(f'{folder}{offername}.json')
        self.assertEqual(offername, jsonread.offer_name)
        self.assertEqual(200, jsonread.amount)
